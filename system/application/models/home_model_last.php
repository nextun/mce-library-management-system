...
Class Home_model extends Model
{
	...
	function initialize($data, $email)
	{
		...
		// insert into web interface
            ...
            $this->db2->query($query_web);
         
            $query = "SELECT id FROM user_info WHERE email = '".trim($email)."'";
         
            $result12 = $this->db2->query($query);
            $result = $result12->row();
    ...
    {
        $this->db2 = $this->load->database('web', true);
        $query_web = 'UPDATE user_info set registration_step= 1 where id='.$id;
        $this->db2->query($query_web);
    ...
    function check_sim_ready($player_id)
	{
			...
			if($result->num_rows > 0)
			{
				...
				if($result->sim_friend_list_ready == 1)
				{
                        ...
                        $query = 'SELECT * FROM USER u, SIM_FRIEND sf WHERE u.PLAYER_ID = '.$player_id.' AND sf.USER_ID = u.ID AND sf.STATUS = 0';
    ...
    {
       $this->db1 = $this->load->database('default', true);
       $query = 'SELECT CREATED FROM USER WHERE PLAYER_ID='.$player_id;
       $result = $this->db1->query($query);
	...
	{
		$this->db1 = $this->load->database('default', true);
        $query = 'SELECT LANGUAGE_ID as language_id  FROM USER WHERE PLAYER_ID = '.$player_id;
		$result = $this->db1->query($query);
		$result = $result->row();
	...
	function achievement_point($player_id, $sim_friend_id)
	{
		...
		$this->db1 = $this->load->database('default', true);
        $query = 'SELECT COUNT(*) AS  NumberOfAchievements FROM USER_ACHIEVEMENTS ua  WHERE ua.USER_ID ='.$user_id;
		$result = $this->db1->query($query);
		$result = $result->row();
	...
	{
		$this->db1 = $this->load->database('default', true);
        $query = 'SELECT ID FROM USER WHERE PLAYER_ID ='.$player_id;
		$result = $this->db1->query($query);
		$result = $result->row();
	...
	{
		$this->db1 = $this->load->database('default', true);
        $query = 'SELECT lt.TEXT AS SkillName, sfs.VALUE AS SkillValue FROM SIM_FRIEND_SKILLS sfs, LIST_VALUE lv, LOCALIZED_TEXT lt WHERE sfs.SIM_FRIEND_ID = '.$sim_friend_id.' AND lv.ID = sfs.SKILL_ID  AND lt.TEXT_ENTRY_ID = lv.TEXT_ENTRY_ID AND lt.LANGUAGE_ID = '.$language_id;
		$result = $this->db1->query($query);
	...
	function get_career($player_id, $simfriend_id)
	{
		...
		$result_careers = $this->db1->query($query);
	...
	{
		$this->db1 = $this->load->database('default', true);
        $query = 'SELECT TRUST FROM SIM_FRIEND WHERE ID ='.$simfriend_id;
		$result = $this->db1->query($query);
		$result = $result->row();
	...
	{
		$this->db1 = $this->load->database('default', true);
        $query = 'SELECT sf.SIMOLEONS AS simoleons FROM SIM_FRIEND sf WHERE sf.ID = '.$sim_friend_id;
		$result = $this->db1->query($query);
	...
	function sim_name($sim_id)
	{
        ...
        $query = ' SELECT FIRST_NAME_ID, LAST_NAME_ID FROM SIM_FRIEND WHERE ID='.$sim_id ;
	...
	{
		$query = 'SELECT sn.NAME as LastName FROM SIM_FRIEND sf, SIM_NAME sn  WHERE sf.ID = '.$sim_friend_id.' AND sn.ID = sf.LAST_NAME_ID';
		$result = $this->db1->query($query);
	...
	*/
	...
	{
		$this->db1 = $this->load->database('default', true);
		$query = 'SELECT LT.TEXT FROM LOCALIZED_TEXT AS LT, LIST_VALUE AS LV, SIM_FRIEND_TRAITS AS SFT WHERE LT.TEXT_ENTRY_ID = LV.TEXT_ENTRY_ID AND LT.LANGUAGE_ID = 1 AND LV.ID = SFT.TRAIT_ID AND SFT.SIM_FRIEND_ID = '.$simfriend_id;
		$result_traits = $this->db1->query($query);
	...
	*/
	...
	{
		$this->db1 = $this->load->database('default', true);
        $query = 'SELECT * FROM USER u, SIM_FRIEND sf WHERE u.PLAYER_ID = '.$player_id.' AND sf.USER_ID = u.ID AND (sf.STATUS = 0 OR sf.STATUS = 1 or sf.STATUS=99)';
		$result = $this->db1->query($query);
		$result = $result->result();
		return $result;
	}
	...
	{
		$this->db1 = $this->load->database('default', true);
        $query = 'SELECT sf.ID as ID FROM SIM_FRIEND sf, USER u WHERE u.PLAYER_ID = '.$player_id.' AND sf.USER_ID = u.ID AND (STATUS = 1 or STATUS=99)';
		$result = $this->db1->query($query);
		$result = $result->row();
		return $result->ID;
    ...
    function get_sim_user_id($user_id)
    {
        ...
        $this->db1 = $this->load->database('default', true);
        $query = 'SELECT ID as id FROM USER WHERE PLAYER_ID = '.$user_id;
    ...
    {
       $this->db1 = $this->load->database('default', true);
       $query = ' SELECT NAME AS last_name FROM SIM_NAME WHERE ID='.$id;
        $result = $this->db1->query($query);
        $result_last_name = $result->row();
    ...
    {
       $this->db2 = $this->load->database('web', true);
       $query = "SELECT email_format, time_zone, email_time, email_freq  FROM user_info WHERE id = '".$player_id."'";
       $result12 = $this->db2->query($query);
       $result = $result12->row();
    ...
    function get_single_pic($sim_id)
    {
        ...
        $this->db1 = $this->load->database('default', true);
        $query = ' SELECT PICTURE_SET_ID FROM SIM_FRIEND WHERE ID='.$sim_id ;
        $result = $this->db1->query($query);
