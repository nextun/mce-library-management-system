...
class Web_model extends Model
{
	...
	function login($data)
	{
           ...
           /* if(preg_match("/@test.test\z/", $username) && $pwd == '1234' )
            ...
            //error_reporting(E_ALL);
                ...
                curl_setopt($ch, CURLOPT_URL, $nucleus_url."/1/users?email={$username}&password={$pwd}");
	...
	function get_user_email_password($user_id)
	{
        ...
        $query = 'SELECT EMAIL as email FROM USER WHERE  PLAYER_ID ='.$user_id;
	...
	{
		$query = 'UPDATE user_info SET registration_step = '.$registration_step.' WHERE id ='.$player_id;
		$this->db2->query($query);
	}
    ...
    function login_initialize($email)
	{
		...
		$query_web = 'INSERT INTO user_info (email, registration_step) VALUES("'.$email.'" , 0)';
